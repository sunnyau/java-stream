...
public class ComplicatedStreamTest {
    ...
    public void filterKey() {
        Map<String, List<Name>> newMap = map.entrySet().stream().filter(p -> p.getKey().equals("Group C"))
    ...
    public void filterKeyWithPredicate() {
        Predicate<Entry<String, List<Name>>> predicateGroupC = p -> p.getKey().equals("Group C");

        Map<String, List<Name>> newMap = map.entrySet().stream().filter(predicateGroupC)
                .collect(Collectors.toMap(p -> p.getKey(),
                        p -> p.getValue()));
    ...
    public void filterValue() {
        Map<String, List<Name>> newMap = map.entrySet().stream().collect(Collectors.toMap(p -> p.getKey(),
    ...
    public void filterValueWithLambdaExpressionPredicate() {
        Predicate<Name> lambdaExpressionPredicate = n -> n.getFirstName().equals("Paul");
    ...
    @Test
    public void flatMap() {
        List<Name> names = map.values().stream().flatMap(l -> l.stream()).collect(Collectors.toList());
        assertEquals(9, names.size());

        // use distinct to get unique names.
        names = map.values().stream().flatMap(l -> l.stream()).distinct().collect(Collectors.toList());
